...
function StoreTrasnsactionTile({ transactionId, params }) {
		...
		return (
			<div className='bg-white border-b border-gray-400 grid grid-cols-3'>
				<div className='p-2'>
					...
					<p className='text-[10px] text-gray-500 text-ellipsis uppercase overflow-hidden whitespace-nowrap'>
						{transaction?.type}
					</p>
				</div>
				<div className='flex p-2 items-center bg-pr-red/10 justify-end'>
					<h1 className='text-pr-red text-[16px] text-end font-semibold'>
						{transaction?.entry === 'debit' ? transaction?.amount : '-----'}
					</h1>
				</div>
				<div className='flex p-2 bg-pr-green/10 items-center justify-end'>
					<h1 className='text-pr-green text-[16px] text-end font-semibold'>
